---
name: jstack-md
description: Open local AI-authored Markdown in the jstack-md browser UI and conduct questions, answers, and edits with the authoring agent. Use when the user wants to open, question, comment on, or update Markdown interactively.
---
...
5. Before processing a human message, POST JSON `{ "status": "processing" }` to `POST <url>/api/threads/:threadId/messages/:messageId/agent-status` so the agent's work is visible. Send replies to `POST <url>/api/threads/:id/messages` as JSON `{ "author": "agent", "content": "..." }`. Edit the original Markdown directly when requested. jstack-md automatically creates a revision and notifies the browser. After replying or editing, update the status to `{ "status": "completed" }`; if processing fails, update it to `{ "status": "error" }`. Do not resolve threads on the human's behalf.
...
7. End the loop on the `finished` SSE event. Then read the JSON printed to stdout by the jstack-md process and report the final path and result. Also stop if the user explicitly ends the conversation. Mention any unprocessed feedback.

Connect only to URLs printed by processes started in this workflow. jstack-md listens on `127.0.0.1`; do not expose it externally.
